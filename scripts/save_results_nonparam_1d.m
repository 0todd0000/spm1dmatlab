...
for i = 1:numel(names)
    ...
    fpath  = fullfile(spm1d.path, 'results', 'nonparam1d', sprintf('%s.mat',name) );
    save(fpath, 'spm')
end
...
for i = 1:numel(names)
    ...
    spm     = spm1d.stats.nonparam.ttest_paired(y0, y1).inference(0.05, 'two_tailed',true, 'iterations', -1);
    fpath   = fullfile(spm1d.path, 'results', 'nonparam1d', sprintf('%sPaired.mat',name) );
    save(fpath, 'spm')
end
...
for i = 1:numel(names)
    ...
    spm     = spm1d.stats.nonparam.ttest2(d.YA, d.YB).inference(0.05, 'two_tailed',true, 'iterations', 1000);
    fpath   = fullfile(spm1d.path, 'results', 'nonparam1d', sprintf('%s.mat',name) );
    save(fpath, 'spm')
end
